...
for Folder in Folder_List:
    ...
    # 判断文件夹是否含有两张图片
    # 创建一个word对象
    document = Document('附件1.docx')
    for jpg in JPG_list:
        # 插入图片必须绝对路径
        jpg_path = os.path.join(Current_Folder_path,Folder,jpg)
        # 插入图片1
        pic1 = document.add_picture(jpg_path, width=Inches(6))  # 添加图片，设置宽度（长度等比例放大缩小）
        ...
        document.paragraphs[-1].paragraph_format.space_after = Pt(30)  # 段后
        # word文档保存
