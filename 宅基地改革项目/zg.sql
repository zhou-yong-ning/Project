...
-- 创建数据透视表
SELECT SUYQR.SUYQRMC,HZ.* FROM
(SELECT leftstr(ZJDDM,18) AS 所有权人代码,
       SUM(CASE WHEN ZJDLYZK=1 THEN 1 ELSE 0 END ) AS 正常使用,
       SUM(CASE WHEN ZJDLYZK=2 THEN 1 ELSE 0 END ) AS 地上房屋倒塌,
       SUM(CASE WHEN ZJDLYZK=3 THEN 1 ELSE 0 END ) AS 地上无房屋,
       SUM(CASE WHEN ZJDLYZK=4 THEN 1 ELSE 0 END ) AS 地上房屋超过一年无人居住,
       SUM(CASE WHEN ZJDLYZK=9 THEN 1 ELSE 0 END ) AS 其他,
       count(*) AS 总计
FROM ZJDLY GROUP BY leftstr(ZJDDM,18)) AS HZ INNER JOIN SUYQR ON SUYQRDM = HZ.所有权人代码;
