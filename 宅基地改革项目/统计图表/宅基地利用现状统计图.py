...
def Statistical_chart(DF):
    ...
    # 设置图表的标题、字号、粗体
    plt.title("农村宅基地利用状况分类统计图", fontsize=15, fontweight='bold')
    # 添加数据标签
...
if __name__ == '__main__':
    ...
    col = '所属区划'      # 指定分割列名称
    print('正在读取文件')
    df = pd.read_csv(r'C:\Users\zhou\Desktop\宗地信息表数据 (15).txt', sep=',', encoding='UTF-8', dtype='object')      # 读取数据
    print('文件读取成功')
