...
print('房屋信息表读取成功')
df2 = pd.read_excel(r"E:\WORK\院宅基地改革项目\宅基地改革\资料收集\房地一体数据20220610\权利人20220610（参考）\保安镇.xlsx", dtype='object')
# 筛选提取户主关系列为‘户主’或者‘本人’的数据表
# df2 = df2[(df2['关系']=='户主') | (df2['关系']=='本人')]
# 删除重复值(保留第一个)
df2.drop_duplicates(subset='宗地代码', inplace=True, keep='first')
# 提取行索引所对应的内容
df0 = df0.loc[:,
      ['宅基地代码', '使用权人代表姓名', '使用权人代表证件类型', '使用权人代表证件号码', '是否本农村集体经济组织成员',
       '户口类型', '权利人类型', '国家/地区', '是否使用权人之间共有', '农户代码']]
df1 = df1.loc[:, ['宅基地代码', '自然栋号']]
df2 = df2.loc[:, ['宗地代码', '图形权利人名称']]
# 列索引重命名
df2.columns = ['宗地号', '房地一体权利人名称']
# 截取宗地号
...
print('数据连接成功（1）')
df2todf1todf0 = pd.merge(df1to0, df2, on='宗地号', how='left')
print('数据连接成功（2）')
# 调整列顺序
df2todf1todf0= df2todf1todf0[
    ['宅基地代码', '宗地号', '农户代码','房地一体权利人名称', '使用权人代表姓名', '是否本农村集体经济组织成员', '权利人类型', '国家/地区',
